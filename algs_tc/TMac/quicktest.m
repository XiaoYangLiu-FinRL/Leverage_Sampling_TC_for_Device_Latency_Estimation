...
Nway = [476,598,15]; % dimension of tensor
r = 10;
coreNway = [r,r,r];
...
%% rank_dec strategy          秩下降方案
opts.alpha_adj = 0;
opts.rank_adj = -1*ones(1,3);
opts.rank_min = 5*ones(1,3);
opts.rank_max = 20*ones(1,3);

EstCoreNway = round(1.25*coreNway);          %EstCoreNway ：初始设置秩偏大(1.25倍的r)
coNway = zeros(1,N);
for n = 1:N
...
for i = 1:3
    X0{i} = randn(Nway(i),EstCoreNway(i));               %产生随机的X0{i}
    Y0{i} = randn(EstCoreNway(i),coNway(i));           %产生随机的Y0{i}
end
...
%% rank_inc strategy      秩上升方案
opts.alpha_adj = 0;
opts.rank_adj = 1*ones(1,3);
opts.rank_min = 5*ones(1,3);
opts.rank_max = round(1.5*coreNway);

EstCoreNway = round(0.75*coreNway);          %EstCoreNway：初始设置秩偏小(0.75倍的r)
coNway = zeros(1,N);
for n = 1:N
    coNway(n) = prod(Nway)/Nway(n);
end
...
opts.X0 = X0; opts.Y0 = Y0;
t0 = tic;
[X_inc,Y_inc,Out_inc] = TMac(data,known,Nway,EstCoreNway,opts);           %秩上升
time = toc(t0);
